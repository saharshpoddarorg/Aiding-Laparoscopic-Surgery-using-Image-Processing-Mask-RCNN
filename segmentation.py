...
#TRANSFERED LEARNING
#imagenet - trained model on imagenet dataset(OEDR dataset of general images): contains weights and parameters...
#coco - trained model on coco dataset(OEDR dataset of general images): contains weights and parameters...
#formats to save NN weights, bias and other paramters - h5, np, npz
...
if init_with == "imagenet":
    ...
    model.load_weights(COCO_MODEL_PATH, by_name=True, 
                       exclude=["mrcnn_class_logits", "mrcnn_bbox_fc", "mrcnn_bbox", "mrcnn_mask"]) #exclude these parameters
elif init_with == "last":
...
#customize existing maskRCNN model w/- coco wts to classify(instrument/ not: instead of actually 80 class_labels)
#edit the layers='heads': final layer to execute the classification
#head: last layer; layers is the argument to edit/mess around as per user req 
...
#learning rate: det. rate of updating wts and bias
